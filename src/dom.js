...
export class Canvas extends Element {
  ...
  getContext(value, contextOptions) {
    if (value === "webgl") {
      ...
      this._webgl = true;
      this._webglResizeExt = this._ctx.getExtension(
...
canvas.Image.prototype = Object.create(canvas.Image.prototype);
  ...
  set: function src(value) {
    ...
    if (typeof value === "string") {
      if (value.match(/^https?/)) {
        ...
        process.nextTick(() => {
          this._src = this.source = Buffer.from(
            val,
